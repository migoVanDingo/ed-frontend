...
export class UserAPI {
  ...
  public static registerUnverified = (token: string) =>
    Requests.doGet(`/api/user/action/register`, ServicePort.USER, {
      Authorization: `Bearer ${token}`,
    })

  public static login = (token: string) =>
    Requests.doPost(null, "/api/user/action/login", ServicePort.USER, {
      Authorization: `Bearer ${token}`,
    })
  ...
  public static getUserSession = () => {
    return Requests.doGet("/api/auth/session", ServicePort.USER)
  }
}
